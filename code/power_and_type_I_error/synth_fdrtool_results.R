#' Calculates p-values using the FNDR empirical null estimation procedure, as implemented in the fdrtool package, 
#' for within-cell-type, between-condition contrasts from synthetic simulations using the model fits from the real-world kidney data 
...
#' We use those parameters/that distribution to get p-values via the FNDR method from the fdrtool package
...
# Load in real data
locfdr_res <- readRDS(file.path(
  processed_data_path,
  "real_hyptest_fdrtool_fndr.rds"
))
...
parameter_df <- distinct(locfdr_res$wald_df_all[, c("contrast_indices",
                                                    "contrast_description",
                                                    "locfdr_sd",
                                                    "locfdr_pi0")])
parameter_df <- parameter_df[!is.na(parameter_df$locfdr_sd), ]
...
tessera_wald_df$wald_pval <- 2 * pnorm(
  abs(tessera_wald_df$wald_stat_t),
  mean = 0,
  sd = tessera_wald_df$locfdr_sd,
  lower.tail = FALSE
)
...
saveRDS(
  ...
  file.path(
    processed_data_path,
    "synth_from_real_hyptest_fitmodel_fdrtool_fndr.rds"
  )
)
