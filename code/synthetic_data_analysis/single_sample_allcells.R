#' Create and run TESSERA and competing methods on a single sample, using ALL cells, from synthetic data based on real-world kidney data
...
out_path <- file.path(path, "singlesample_allcells_output_onlyinteract")
...
drop_cols <- sort(unique(c(drop_cols_0, drop_cols_int)))
